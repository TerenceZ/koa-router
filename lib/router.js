    ...
    function *next() {
      ...
      if (++index >= routes.length) {
        ...
        if (!called && !context.response._explicitStatus) {
          ...
          // If we set the status using `ctx.status`, it will make the explicitStatus true,
          // which means when we writes `cyx.body` in upstream, it will not auto modify the `ctx.status`.
          // So here we just wirte the status in the `ctx.res.statusCode` directly.
